        ...
        try
        {
            ...
            case alice::PacketType::PULL: {
                try {
                    ...
                    std::memcpy(&source_ip, packet.payload.data(), sizeof(uint32_t));
                    std::memcpy(&source_port, packet.payload.data() + sizeof(uint32_t), sizeof(uint16_t));
                    source_port = ntohs(source_port); // Convert to host byte order
                    Logger::log(LogLevel::INFO, "Received PULL request from " + source_port + ":" + std::to_string(source_port));

                    std::string source_ip_str = convertVectorToIp({ 
                        static_cast<uint8_t>((source_ip >> 24) & 0xFF), 
                        static_cast<uint8_t>((source_ip >> 16) & 0xFF), 
                        static_cast<uint8_t>((source_ip >> 8) & 0xFF), 
                        static_cast<uint8_t>(source_ip & 0xFF) 
                    });

                    Logger::log(LogLevel::INFO, "Received PULL request from " + source_ip_str + ":" + std::to_string(source_port));
                    ...
                    payload_stream << R"({
                        "sensor_id": "edge001",
                        "location": {
                            "latitude": )" << latitude_ << R"(,
                            "longitude": )" << longitude_ << R"(,
                            "altitude_meters": )" << altitude_ << R"(
                        },
                        "timestamp": ")" << std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()) << R"(",
                        "data": {
                            "temperature_celsius": )" << (20.0 + static_cast<float>(rand()) / (RAND_MAX / (40.0 - 20.0))) << R"(,
                            "humidity_percentage": )" << (30.0 + static_cast<float>(rand()) / (RAND_MAX / (70.0 - 30.0))) << R"(,
                            "co2_ppm": )" << (350.0 + static_cast<float>(rand()) / (RAND_MAX / (450.0 - 350.0))) << R"(,
                            "smoke_density": {
                                "concentration_micrograms_per_cubic_meter": )" << (100.0 + static_cast<float>(rand()) / (RAND_MAX / (300.0 - 100.0))) << R"(,
                                "detected": )" << (rand() % 2 == 0 ? "true" : "false") << R"(
                            }
                        },
                        "metadata": {
                            "battery_level_percentage": )" << (50.0 + static_cast<float>(rand()) / (RAND_MAX / (100.0 - 50.0))) << R"(,
                    ...
                    uint16_t crc = alice::Packet::crc16(payload_data);

                    // Construct response packet
                    alice::Packet response_packet(
                        id_,                        // Source ID
                        packet.source_id,           // Destination ID
                        alice::PacketType::DATA,    // Packet type
                        1,                          // Priority
                        packet.sequence_number + 1, // Sequence number
                        payload_data,               // Payload
                        crc,                        // CRC
                        1                           // Payload type (1 for JSON)
                    );
                    ...
                    std::istringstream ip_stream(ip_table_->get_ip(packet.source_id));
